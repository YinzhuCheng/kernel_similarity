...
def kernel_similarity_scores_normalized(
    ...
    with torch.no_grad():
        k_qq = (
            kernel(query_tensor, query_tensor)
            .evaluate()
            .squeeze()
        )
        k_qq = torch.clamp(k_qq, min=eps)
    ...
    for start in range(0, doc_matrix.shape[0], batch_size):
        ...
        with torch.no_grad():
            # k(q, d): (1, B)
            k_qd = kernel(query_tensor, batch).evaluate().squeeze(0)

            # k(d, d): (B,)
            k_dd = kernel(batch, batch).evaluate().diag()
            k_dd = torch.clamp(k_dd, min=eps)

            # normalized kernel (cosine-like)
            batch_scores = k_qd / torch.sqrt(k_qq * k_dd)

        scores[start:end] = (
            batch_scores.detach().cpu().numpy().astype(np.float32)
        )
